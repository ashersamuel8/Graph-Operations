...
int main ( int argc, char* argv[] ) {
    ...
    for (unsigned iter=0; iter<graphNodeCount-1; iter++) {
        ...
        for (graphNode_t source=0; source<graphNodeCount; source++) {       //to find the minimum weight
            if (parents[source]!=-1) { // if already visited
                /* ... */
                AdjacencyListNode* x = adjacencyList[source].next;
                while (x) {
                  if (x -> weight < minWeight && parents[x -> graphNode] == -1){
                    minWeight = x -> weight;
                    minDest = x -> graphNode;
                    minSource = source;
